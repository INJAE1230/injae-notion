...
- 대상별 분리: "김대리랑 미팅, 박과장님한테 보고" → 2건 (대상마다 동사/행동이 각각 다름)
- 괄호 안 별도 업무: "개발(테스트 포함)" → 1건 (content에 테스트 포함 기재)

예외 (분리하지 않고 1건으로 묶기):
- 쉼표로 대상(사람/사물)을 나열한 뒤 동사가 문장 끝에 한 번만 오면, 그 동사는 나열된 대상 전체에 공통으로 적용되는 것이므로 1건으로 묶는다.
  예: "본부장님, 부장님, 호정이모, 다혜씨 PC SSD 교체작업 함" → 1건 (동사 "교체작업 함"이 한 번뿐이고 4명 모두의 공통 작업)
  title: "PC SSD 교체작업" / content에 대상 전원 나열: "본부장님·부장님·호정이모·다혜씨 PC SSD 교체 완료"
  판별법: 쉼표로 끊긴 각 구간 뒤에 동사가 각각 붙어있으면 분리(대상별 분리), 마지막에 동사가 한 번만 있으면 병합(이 예외)
